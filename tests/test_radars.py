...
import pytest
from src.radars.models import Coordinates, Enemy, Scan, RadarInput
...
def test_enemy():
    type = "soldier"
    number = 10
    enemy = Enemy(type, number)
    assert enemy.type == type
    assert enemy.number == number
...
def test_scan():
    ...
    assert scan.coordinates.x == coordinate.x
    assert scan.coordinates.y == coordinate.y
    assert scan.enemies.type == enemy.type
    assert scan.enemies.number == enemy.number
    assert scan.allies == 0


def test_radar_input():
    type = "soldier"
    number = 10
    enemy = Enemy(type, number)
    x = 0
    ...
    radar = RadarInput(input["protocols"], input["scan"])
    assert radar.scan[0].coordinates.x == coordinate.x
    assert radar.scan[0].coordinates.y == coordinate.y
    assert radar.scan[0].enemies.type == enemy.type
    assert radar.scan[0].enemies.number == enemy.number
    assert radar.scan[0].allies == 0
    assert json.dumps(radar.get_coords()) == json.dumps({"x": 0, "y": 40})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"protocols": ["closest-enemies"],
          "scan": [{"enemies": {"number": 10, "type": "soldier"}, "coordinates": {"y": 35, "x": 5}},
        ...
        ({"protocols": ["assist-allies"],
          "scan": [{"enemies": {"number": 10, "type": "soldier"}, "allies": 3, "coordinates": {"y": 35, "x": 5}},
        ...
        ({"protocols": ["closest-enemies", "avoid-mech"],
          "scan": [{"coordinates": {"x": 0, "y": 1}, "enemies": {"type": "mech", "number": 1}},
...
def test_full_cases(input, expected):
    scanner = RadarInput(input["protocols"], input["scan"])
    assert json.dumps(scanner.get_coords()) == json.dumps(expected)
